...
const (
	...
	GetUserRoleQuery = `SELECT users.user_role FROM users WHERE meta().id = $1`

	GetUserIdByUsernameQuery = `SELECT meta().id FORM users WHERE users.username= $1`

	UpdateUserQuery = `UPDATE users SET username=$1, pass=$2, user_role=$3, created_at=$4 WHERE meta().id = $5`
